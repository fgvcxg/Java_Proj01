...
public class String_Method {
	...
	public static void main(String[] args) {
		...
		System.out.println(str1.indexOf('a'));		//0에서부터 7번째
		System.out.println(str1.lastIndexOf('a'));	//뒤에서부터 시작해서 뒤에 있는 a를 인식 9
		
		System.out.println(str1.indexOf('a',8));	//8번부터 a를 검색 : 9
		System.out.println(str1.lastIndexOf('a',8));//뒤에서부터 시작하는데 8번부터 시작 : 7
		
		System.out.println("======================");
		
			//문자열을 검색해서 방번호를 출력
		System.out.println(str1.indexOf("Java"));	//6번방에서부터 시작됨
		System.out.println(str2.indexOf("하세요"));
		System.out.println(str1.lastIndexOf("ello"));
		
			//문자열 검색에 실패시 : -1 값을 출력
		System.out.println(str1.indexOf("bye"));	//찾는 문자열이 없으면 -1이 출력됨
		System.out.println();
		
		//3. 문자열 변환 및 문자열 연결 : String.valueOf(), concat()
		// String.valueOf() : 기본자료형을 ==> String 자료형으로 변환
		
		//String str3 = 2.3;	//오류발생 : 2.3( Double 형 데이터를 String 에 담으니 오류)
		String str3 = String.valueOf(2.3);
		//String str4 = true;		//boolean 차입을 String 변수에 저장시 오루 발생		
		...
		//concat() : + 연산자와 같이 문자열과 문자열을 연결시켜주는 메소드
		...
		//getByte() : 문자열을 1글짜씩 잘라서 byte 배열에 저장, 아스키 코드값이 출력
		...
		//5. 문자열 수정, 대문자로 변환 : toUpperCase(), 소문자로 변환 : toLowerCase();
		...
		System.out.println(str13.substring(0,5));	//0번에서 시작해서 4번 방 앞까지 짤림
			//모든 프로그램에서 시작은 포함, 끝은 포함
