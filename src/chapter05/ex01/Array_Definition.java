...
public class Array_Definition {
	...
	public static void main(String[] args) {
		/*
		 	자료형 : 변수를 선언시 변수에 할당할 값을 정의
		 		...
		 		- 참조자료형 : 배열, 열거, 클래스 / 인터페이스 
		 			...
		 			- 변수에 들어갈 값을 Heap 영역에 저장됨
		 	변수 	- 변수에는 하나의 값만 저장됨
		 		- 초기값을 할당후 변수를 사용
		 	배열 	- 하나의 배열 변수에 많은 값을 저장 할 수 있다.
		 		- 배열의 특징 :	1. 배열변수의 동일한 자료형의 값만 저장할 수 있다.(int로 선언했다면 int 형만 가능)
		 					2. 배열은 방의 갯수가 정의되면 수정이 불가능
		 		- 배열의 초기값을 할당하지 않더러고 강제로 초기값이 할당
		 		- Heap 공간에는 값이 반드시 할당되어야 함
		 			boolean(false), 정수형[byte, short, int, long](0), 실수[float, double](0.0)
		...
		int c;
		int d;
		//int sum = c+d; // c, d 변수에 초기 값을 넣지 않고 사용했기에 오류
		...
		//int[] : 정수 값만 배열에 저장 가능
		//array1 : 배열 변수, array1 변수 명은 Stack 공간에 저장이되고, Heap 영역에 주소값이 들어가 있다
		...
		array9 =new int[]{111,222,333,444,555};
