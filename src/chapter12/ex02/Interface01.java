...
class Bb implements B{
	@Override
	public void abc() {	//오류 발생 : 인터페이스는 기본적으로 public 이기 때문에 상속을 받은
							//클래스는 반드시 Override 할때 public을 넣어야 한다
							//void abc() <== 이렇게 하면 오류가 난다는 것이다
		System.out.println("B 인터페이스를 구현한 메소드");
		
	}
}

//추상 클래스 : 추상 메소드를 포함한 클래스, 추상 클래스내의 추상메소드는 abstract 키를 생략하면 안됨
abstract class C {
	...
	//추상 클래스 내의 추상메소드는 abstract 를 무조건 붙여야 한다. 생략하면 안된다
	abstract void abc();
}
...
class Cc extends C{
	@Override
	void abc() {
		System.out.println("C 추상클래스를 구현한 메소드 : Cc 클래스의 오버라이딩된 메소드");	
	}
}
		...
		/*
		 	인터페이스(Interface) : 내부와 외부를 연결시켜주는 접점, 콘센트
		 		...
		 		- 모든 필드는 public static final 이 생략됨
		 			: 값을 수정할 수 없다
		 		- 모든 메소드는 public abstract 가 생략됨 (예외, default, static 메소드 : ver1.8 부터)
		 			: 모든 메소드는 선언부만 있어야 한다. 구현부가 있으면 안됨 :  {} 이거 있으면 안됨
		 			: 예외, default, static 메소드는 인터페이스에서 유일하게 구현부를 가지는 메소드
		 			
		 		- 하나의 클래스는 하나의 부모 클래스를 상속해야 한다. 자바는 다중 상속 불가
		 		- 하나의 클래스는 여러 개의 인턴페이스를 다중 상속이 가능하다.
		 	
		 */
		
		//1. interface는 객체화할 수 없다. 구현되지 않는 메소드가 있기 때문에
//		A a = new A();
//		B b = new B();
		
		//2. 타입 지정은 가능하다 < ==== (중요함. 많이 사용)
		A a1 = new Aa();	// A : 인터페이스, Aa : 인터페이스를 구현한 클래스
											//(객체화를 할 수 있는 것은 콘크리트 클래스만 가능)
		a1.abc();
...
//		System.out.println(C.m);	//인스턴스 필드 : 객체 생성후 호출 가능
		
		c2.m = 44;
		System.out.println(c2.m); 	//값 수정이 가능
