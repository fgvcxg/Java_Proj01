...
//추상 메소드를 구현하는 방법 2가지
	...
	//2. 익명클래스를 생성해서 출력 : 한번 사용하고 버릴 경우, 안드로이드 폰 개발할때, 이벤트 처리할때 사용.
			//추상 클래스의 구현한 자식 클래스 생성없이 바로 사용이 가능
...
//2. 두번째 방법 : 추상 클래스를 구현한 클래스 없이 바로 출력 : 익명의 클래스를 사용해서 출력
abstract class C{		
	abstract void run();	
}
...
class B extends A{		//1. 자식 클래스에서 추상클래스의 메소드를 구현한 클래스를 생성
	@Override
	void cry() {
		System.out.println("모든 동물은 웁니다(자식 클래서에서 추상메소드를 완성)");
	}
}
...
//		A a = new A();	//A 클래스는 추상클래스이기 때문에 객체화가 불가능하다
		...
		//두번째 방법 : 구현 클래스 없이 바로 출력하는 방법(익명 클래스 사용)
			//익명 클래스 : 
		C c1 = new C() {
			//C : 추상 클래스의 구현되지 않은 메소드를 구현함.			
			@Override
			void run() {
