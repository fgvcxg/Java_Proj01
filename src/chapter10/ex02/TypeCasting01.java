...
public class TypeCasting01 {
	...
	public static void main(String[] args) {
		...
		//업캐스팅 : 자식 객체를 만들어서 부모 타입으로 정의, 자동으로 일어남
		...
		//2. 다운 캐스팅 : Animal(부모 타입) --> Tiger (자식타입), 수동으로 명시
			//a1 : Tiger 필드, 메소드 + Animal 필드와 메소드도 포함
			//a1 : Animal 타입 --> t2( Tiger 타입)으로 다운 캐스팅
		
		Tiger t2 = (Tiger) a1;
		//t2는 Animal, Tiger 의 필드와 메소드를 모두 접근 가능
		//t2.name = "호랑이2";	//위에서 선언 했기에 안하면 a1 에서 했던 것들이 나온다
		t2.age = 6;
		...
		//t3 라는 객체에 Animal을 포함하면 true
		System.out.println(t3 instanceof Tiger);		//true
		//t3 라는 객체에 Tiger을 포함하면 true
		System.out.println(t3 instanceof TigerChild);	//false
		//t3 라는 객체에 TigerChild을 포함하면 true
		
		
		//다운 캐스팅 할때에는 반드시 instanceof 를 사용해야 한다.
		//안쓰면 실행시에 문제가 나올 수도 있다.
		if(t3 instanceof TigerChild) {	
			//객체를 다운 캐스팅 할때는 반드시 instanceof 로 그 객체가 포함이 되어있는지 확인하고 실행해야한다.
		...
		//3. a3를 TigerChild 로 다운 캐스팅
		
		TigerChild t6 = null;	//블락밖에서 변수 선언
		if(t3 instanceof TigerChild) {
			t6 = (TigerChild) t3;
		}
