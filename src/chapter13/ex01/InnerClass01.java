...
/*
 	이너클래스(Inner class) : 클래스 내부의 클래스
 		1. 인스턴스 이너 클래스 : static 키가 붙지 않는 내부 클래스
 			//외부 클래스의 객체를 먼저 생성후에 사용가능
 		2. 정적(static) 이너클래스 : static 키가 붙어있는 내부 클래스
 		3. 지역 이너클래스 : 메소드 내부의 선언된 이너클래스
 		
 	이너클래스 특징 : 
 		1. Outer Class의 접근제어자의 제한없이 모두 접근이 가능(필드, 메소드)
 		2. 다른 객체를 생성하지 않고, 내부에 객체를 만들어서 빠르게 사용하기 위해서
 			클래스의 필드와 메소드는 객체화 해야 사용이 가능하다
 		
 */

class A{	//Outer class : 필드, 메소드에 접근 제거자가 붙어 있고, 
				//Inner class를 생성하면 Outer class의 접근제어자와 상관없이 접근
	
	public int a = 3;		//다른 패키지에서 접근가능, 같은 패키지에서도 접근 가능
	protected int b = 4;	//다른 패키지에서 접근가능(상속), 같은 패키지에서도 접근가능
	int c = 5;				//default, 다른 패키지에서는 접근이 불가능, 같은 패키지에서만 접근가능
	private int d = 6;		//다른 패키지, 같은 패키지에서 접근 불가능
