...
public class TryWithResource01 {
	...
	public static void main(String[] args) {
		...
		//일반예외 (Catched Exception) : 프로그램 작성시 컴파일 단계에서 오류
			//반드시 예외를 처리해야한다.
			//1. 예외를 남에게 전가	: throws 를 사용ㅇ해서 호출하는 쪽에서 처리하도록 미루는 방법
					//메소드 블락에서 적용됨, 메소드를 호출하는 곳에소 예외 처리를 해야함
		...
		try {
			...
			//일반적으로 객체를 사용후 finally 블락에서 객체를 제거함
				//객체를 제거할 때도 예외 처리가 되어야 됨.
			if(ir1 != null) {	//객체가 null일때는 제거하지 않고, 객체가 null이 아닐때 제거함.
				try {
