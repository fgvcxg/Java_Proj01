...
public class TryWithResource02 {
	...
	public static void main(String[] args) {
		//TryWithResource : Java ver1.7
		// 일반적인 try 구문 : try { } catch(Exception e) { }
		//Try With Resource :
			//		try( 객체선언[Resource] ){ } catch(Exception e) { }
			//자동으로 객체를 close()
			//자동으로 객체를 제거하기 위해서는 AutoClosable() 이라는 인터페이스를 구현 클래스만이 자동으로 제거된다
		...
		try (InputStreamReader ir1 = new InputStreamReader(System.in)){
			...
			System.out.println("입력한 글자는 " + input + "입니다");
		...
		//try ( 객체선언 및 생성 )
		//finally 블락에 close() 메소드를 사용하지 않더라도 자동으로 close()된다
